...
def _build_booking_service(
    ...
    repository = repository or create_autospec(
        BookingRepository, instance=True)
...
class TestBookingServiceCreate:
    ...
    async def test_create_booking_with_existing_ticket_counter(
        ...
        booking_data = BookingCreate(
            event_id=11, user_email="user@example.com")
        event = {"id": 11, "title": "Concert",
                 "price": "1500.00", "total_tickets": 5}
        event_tickets = make_event_tickets(event_id=11, available_tickets=3)
        booking = make_booking(
            booking_id=7, user_id=42, event_id=11,
            price_at_booking=Decimal("1500.00"))

        async def fake_get_event(event_id: int) -> dict:
            """Возвращает тестовые данные мероприятия."""
            assert event_id == 11
            return event

        monkeypatch.setattr(
            "src.services.booking_service.get_event", fake_get_event)
        send_task = MagicMock()
        monkeypatch.setattr(
            "src.services.booking_service.celery_client.send_task", send_task)
    ...
    async def test_create_booking_initializes_local_ticket_counter_on_first_booking(  # noqa: E501
        self,
        monkeypatch,
        mock_session,
        make_booking,
        ...
        event = {"id": 5, "title": "First sale",
                 "price": "999.99", "total_tickets": 2}
        booking = make_booking(booking_id=10, user_id=1,
                               event_id=5, price_at_booking=Decimal("999.99"))
...
class TestBookingServiceGetBooking:
    ...
    async def test_get_booking_returns_booking_for_owner(
    ...
    async def test_get_booking_raises_403_for_foreign_booking(
            self, mock_session, make_booking):
        """Проверяет ошибку 403."""
        service, repository, _ = _build_booking_service(session=mock_session)
        repository.get_by_booking_id.return_value = make_booking(
            booking_id=1, user_id=100, event_id=22)
...
class TestBookingServiceGetMyBookings:
    """Тесты списка бронирований пользователя."""
    async def test_get_my_bookings_returns_repository_result(
            self, mock_session, make_booking):
        """Проверяет ожидаемый результат."""
        service, repository, _ = _build_booking_service(session=mock_session)
...
class TestBookingServiceCancelBooking:
    ...
    async def test_cancel_booking_marks_booking_as_cancelled_and_returns_ticket(  # noqa: E501
        self,
        mock_session,
        make_booking,
        make_event_tickets,
        ...
        """Проверяет ожидаемый результат."""
        service, repository, _ = _build_booking_service(session=mock_session)
        booking = make_booking(booking_id=3, user_id=1,
                               event_id=9, status=BookingStatus.CONFIRMED)
    ...
    async def test_cancel_booking_raises_409_when_already_cancelled(
            self, mock_session, make_booking):
        """Проверяет ошибку 409."""
        service, repository, _ = _build_booking_service(session=mock_session)
        repository.get_by_booking_id.return_value = make_booking(
