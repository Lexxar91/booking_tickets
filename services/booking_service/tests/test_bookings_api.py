...
from src.api.v1.bookings import (
    booking_create,
    cancel_booking,
    get_booking,
    get_my_bookings,
...
class TestBookingsApi:
    ...
    async def test_create_booking_returns_created_booking_and_commits(
            self, make_booking):
        """Проверяет успешный сценарий с commit."""
        session = AsyncMock()
        session.commit = AsyncMock()
        service = create_autospec(BookingService, instance=True)
        service.create.return_value = make_booking(
    ...
    async def test_get_my_bookings_returns_service_result_without_commit(
            self, make_booking):
    ...
    async def test_get_booking_returns_requested_booking(self, make_booking):
        """Проверяет ожидаемый результат."""
        service = create_autospec(BookingService, instance=True)
        service.get_booking.return_value = make_booking(
            booking_id=5, user_id=42, event_id=9)
    ...
    async def test_cancel_booking_returns_updated_booking_and_commits(
            self, make_booking):
        """Проверяет успешный сценарий с commit."""
        session = AsyncMock()
        session.commit = AsyncMock()
